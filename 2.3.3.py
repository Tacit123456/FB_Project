...
def animate_flow_evolution(U, a, Gamma_values, grid_resolution=100, interval=100):
    ...
    def update(frame):
        ...
        contourf = ax.contourf(
            *np.meshgrid(np.linspace(-2 * a, 2 * a, grid_resolution),
                         np.linspace(-2 * a, 2 * a, grid_resolution)),
            precomputed_psi[frame],
            levels=20, cmap=cmap, alpha=0.5
        )
        contour = ax.contour(
            *np.meshgrid(np.linspace(-2 * a, 2 * a, grid_resolution),
                         np.linspace(-2 * a, 2 * a, grid_resolution)),
        ...
        if abs(Gamma) <= 4 * np.pi * U * a:
            ...
            ax.text(0, 1.5 * a, 'Flow Separation Detected',
                    color='red', ha='center', va='bottom', fontweight='bold')

        return ax

    # 生成动画
    ani = FuncAnimation(fig, update, frames=len(Gamma_values),
